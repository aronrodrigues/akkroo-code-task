	...
	function sendEmail(toAddress, templateName, templateVars, cb) {
		...
		setTimeout(function() {
			var templates = {
				...
				'salaryEmail' : function(templateVars) {
					return [
						'Hello Aron',
						templateVars.name + ' really liked your job and offered ' + templateVars.salary
					].join('\n');
				},  
			};
			...
			cb(template); // Fixed (The cb must be called inside the async op);
			return true;
		}, 1);
